...
def animate(i, IRSamples, RedSamples, timeLen, IRLine, RedLine, IRRange, RedRange, irax, redax):
    ...
    if(ir != 0 and red != 0):
        ...
        if(red < RedRange[1] - 500):
            RedRange[1] -= 100
            redax.set_ylim(RedRange)

        if(red > RedRange[0] + 500):
            RedRange[0] += 100
            redax.set_ylim(RedRange)
...
def plot ():
    ...
    IRRange = [1000, 2000]  # Range of possible Y values to display
    RedRange = [1000, 2000]  # Range of possible Y values to display
    ...
    plt.legend([IRLine, RedLine], ['IE', 'RED'])
